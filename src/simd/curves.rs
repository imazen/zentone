...
//!
//! `bt2446a_tier` upgraded to `f32x16 + v4(cfg(avx512))` in the BT.2446-A
//! perf pass — `F32x16Convert` IS impl'd for `X64V4Token` (and V3 as 2× AVX2),
//! so the kernel now resolves to real AVX-512 on Zen 4 / Sapphire Rapids
//! hardware and to 2× AVX2 vectors on V3. Adding `define(f32x16)` to the
//! other kernels here is a follow-up.
...
/// Sqrt-substituted 2-piece monomial polynomial for `x^(1/2.4)` on the input
/// gamma-encode stage of `Bt2446A`, calibrated for **8-bit sRGB output**.
///
/// Substitution: fit `g(z) = z^(2/2.4) ≈ z^0.833` over `z = √x ∈ [0, √11]`.
/// The sqrt flattens the singular tangent at `x = 0` (`f'(0) = ∞` becomes
/// `g'(0) = 0`), so the polynomial is well-behaved. The 2-piece split at
/// `z = 0.07` (z-space, i.e. `x ≈ 0.0049`) separates the fast-curving
/// near-zero region (deg 7 fit on `z ∈ [0, 0.07]`) from the bulk (deg 10
/// fit on `z ∈ [0.07, √11]`).
///
/// **Polynomial max absolute error**: 3.44e-4 (vs `libm::powf(x, 1/2.4)`,
/// measured at 1M dense samples over `x ∈ [0, 11]`).
///
/// **Kernel output max error**: 5.21e-4 in linear-light space (after the
/// Cb/Cr subtractive amplification in BT.2446-A's Hunt color correction,
/// measured by the prior pass on the existing `bt2446a_strip_simd_matches_per_pixel`
/// 9090-comparison grid).
///
/// **8-bit sRGB half-LSB threshold**: 1/(2·255) = 1.96e-3 in normalized
/// linear space — well above both the polynomial error and the
/// kernel-amplified error. The relaxed-precision output is byte-identical
/// to the spec-strict path after sRGB-u8 encoding for ~all pixels.
///
/// Coefficients are in **ascending degree order**:
/// `p(z) = c[0] + c[1]·z + c[2]·z² + ... + c[N-1]·z^(N-1)`.
///
/// Generated by `examples/fit_pow_inv_24.rs` (run with
/// `cargo run --example fit_pow_inv_24 --release --features __polyfit-tools`).
/// Re-run produces byte-identical numbers; the polyfit fit is deterministic
/// given the same domain + degree + sample count.
///
/// The split parameter `eps = 0.07` in `fit_pow_inv_24.rs` is already in
/// z-space (the example's `near_coeffs` is `fit_plain_poly_for(ref_pow_sqrt,
/// 0.0, eps, 7)` — fit over `z ∈ [0, 0.07]`). The kernel compares the
/// per-lane `z = √x` directly against this value.
const POW_INV24_SPLIT_Z: f32 = 0.07;

/// Near-zero branch coefficients (`z < POW_INV24_SPLIT_Z`), degree 7.
const POW_INV24_NEAR: [f32; 8] = [
    3.441_903_7e-4,
    2.753_522_9,
    -1.011_498_3e2,
    5.187_170_4e3,
...
/// Bulk branch coefficients (`z ≥ POW_INV24_SPLIT_Z`), degree 10.
const POW_INV24_BULK: [f32; 11] = [
    1.536_768_6e-2,
    1.424_879_8,
    -1.327_976_5,
    2.264_049_5,
...
///
/// Optimizations vs the prior f32x8 + V3-only kernel:
...
///    vertical tangent near 0. Polynomial fits over `[0, 4]` give 10%+ error.
///    We keep `pow_midp_unchecked(1/2.4)` for the input transfer.
...
pub(crate) fn bt2446a_tier(
    ...
    for chunk in &mut iter {
        ...
        // 1/2.4 gamma encode → Y'. `pow_midp_unchecked` skips the
        // zero/negative/inf/NaN blend chain that `pow_midp` adds for IEEE
        // edge cases. Inputs are guaranteed ≥ pos_eps (positive normal), so
        // the underlying log2/exp2 polynomial path is correct.
        ...
        // pow(rho_sdr, y_c) = exp2(y_c * log2(rho_sdr)). `y_c * log2_rho_sdr`
        // is bounded (y_c ≤ ~1, log2_rho_sdr ≈ 2.5 at 100 nit SDR) so
        // exp2_midp_unchecked is in range.
...
/// Scalar evaluator for the 2-piece sqrt-substituted polynomial.
/// Used by the scalar tail of `bt2446a_u8_tier` and by the
/// `Bt2446A::map_rgb_for_u8` per-pixel path so the tail and the SIMD
/// body produce numerically equivalent output.
#[inline]
pub(crate) fn eval_pow_inv24_2piece_scalar(x: f32) -> f32 {
    let z = sqrtf(x.max(0.0));
    let coefs: &[f32] = if z < POW_INV24_SPLIT_Z {
        &POW_INV24_NEAR
    } else {
        &POW_INV24_BULK
    };
    // Horner on z, highest degree first.
    let mut y = coefs[coefs.len() - 1];
    for &c in coefs[..coefs.len() - 1].iter().rev() {
        y = y.mul_add(z, c);
    }
    y
}

/// 16-wide BT.2446-A SIMD kernel **calibrated for 8-bit sRGB output**.
///
/// Identical to [`bt2446a_tier`] except the three input-gamma-encode
/// `pow_midp_unchecked(1/2.4)` calls are replaced with a sqrt-substituted
/// 2-piece monomial polynomial of `√x` (see [`POW_INV24_NEAR`] /
/// [`POW_INV24_BULK`] / [`POW_INV24_SPLIT_Z`] constants above).
///
/// **Precision tradeoff**:
/// - Polynomial error: 3.44e-4 over `x ∈ [0, 11]` (vs `libm::powf` reference).
/// - Kernel output error: 5.21e-4 in linear-light space (after Cb/Cr amplification).
/// - 8-bit sRGB half-LSB threshold: 1.96e-3.
///
/// Output is **byte-identical** to [`bt2446a_tier`] after sRGB-u8 encoding
/// for ~all pixels — the 5.21e-4 linear-light error is ~0.13 LSBs at 8-bit,
/// well below the 0.5-LSB rounding threshold.
///
/// **When to use**: callers whose output is destined for sRGB-u8 (JPEG,
/// PNG-8, u8 buffer). Saves the three `pow_midp_unchecked(1/2.4)` calls
/// per pixel — each ~20 cycles of polynomial math — at the cost of one
/// `sqrt` + two parallel Horner chains gated by a single lane-wise
/// comparison.
///
/// **When NOT to use**: 16-bit PNG output, PQ-correct round-tripping, or
/// any reference output for spec validation. Use [`bt2446a_tier`] instead.
#[archmage::magetypes(define(f32x16), v4(cfg(avx512)), v3, neon, wasm128, scalar)]
pub(crate) fn bt2446a_u8_tier(
    token: Token,
    row: &mut [[f32; 3]],
    rho_hdr: f32,
    inv_log_rho_hdr: f32,
    ...
    // 2-piece sqrt-substituted polynomial for the input gamma encode (`x^(1/2.4)`).
    // See `POW_INV24_*` constants above. Splatted once per kernel call;
    // the FMA pipeline keeps them resident across the 16-pixel loop body.
    let split_z = f32x16::splat(token, POW_INV24_SPLIT_Z);
    let nc0 = f32x16::splat(token, POW_INV24_NEAR[0]);
    let nc1 = f32x16::splat(token, POW_INV24_NEAR[1]);
    let nc2 = f32x16::splat(token, POW_INV24_NEAR[2]);
    let nc3 = f32x16::splat(token, POW_INV24_NEAR[3]);
    let nc4 = f32x16::splat(token, POW_INV24_NEAR[4]);
    let nc5 = f32x16::splat(token, POW_INV24_NEAR[5]);
    let nc6 = f32x16::splat(token, POW_INV24_NEAR[6]);
    let nc7 = f32x16::splat(token, POW_INV24_NEAR[7]);
    let bc0 = f32x16::splat(token, POW_INV24_BULK[0]);
    let bc1 = f32x16::splat(token, POW_INV24_BULK[1]);
    let bc2 = f32x16::splat(token, POW_INV24_BULK[2]);
    let bc3 = f32x16::splat(token, POW_INV24_BULK[3]);
    let bc4 = f32x16::splat(token, POW_INV24_BULK[4]);
    let bc5 = f32x16::splat(token, POW_INV24_BULK[5]);
    let bc6 = f32x16::splat(token, POW_INV24_BULK[6]);
    let bc7 = f32x16::splat(token, POW_INV24_BULK[7]);
    let bc8 = f32x16::splat(token, POW_INV24_BULK[8]);
    let bc9 = f32x16::splat(token, POW_INV24_BULK[9]);
    let bc10 = f32x16::splat(token, POW_INV24_BULK[10]);
    ...
    // Per-lane evaluator: `z = √x`; both branches evaluated in parallel
    // via **Estrin's method** to keep the FMA dep chain shallow; blend
    // per lane on `mask = z < split_z`.
    //
    // Near branch (deg 7, same shape as the EOTF Estrin chain):
    //   p7(z) = ((c7·z + c6)·z² + (c5·z + c4))·z⁴ + ((c3·z + c2)·z² + (c1·z + c0))
    //   depth = 3 FMAs.
    //
    // Bulk branch (deg 10): Estrin via z², z⁴, z⁸:
    //   A_i = c_{2i} + c_{2i+1}·z   (i = 0..4, depth 1)
    //   A5 = c10 (singleton)
    //   B_j = A_{2j} + A_{2j+1}·z²  (j = 0..2, depth 2)
    //   C = B0 + B1·z⁴              (depth 3)
    //   p10 = C + B2·z⁸             (depth 4)
    //   Critical-path latency: **4 FMAs** vs Horner's **10**.
    //
    // Both branches share `z²` and `z⁴`; computing `z⁸` only matters for
    // the bulk branch. The compiler hoists shared work automatically.
    let eval_pow_inv24 = |x: f32x16| -> f32x16 {
        let z = x.sqrt();
        let z2 = z * z;
        let z4 = z2 * z2;
        let z8 = z4 * z4;

        // Near branch: deg-7 Estrin (depth 3 FMAs).
        let n_h7 = nc7 * z + nc6;
        let n_h5 = nc5 * z + nc4;
        let n_h3 = nc3 * z + nc2;
        let n_h1 = nc1 * z + nc0;
        let n_hi = n_h7 * z2 + n_h5;
        let n_lo = n_h3 * z2 + n_h1;
        let yn = n_hi * z4 + n_lo;

        // Bulk branch: deg-10 Estrin (depth 4 FMAs).
        let a0 = bc1 * z + bc0;
        let a1 = bc3 * z + bc2;
        let a2 = bc5 * z + bc4;
        let a3 = bc7 * z + bc6;
        let a4 = bc9 * z + bc8;
        // a5 = bc10 (singleton — feeds the final merge).
        let b0 = a1 * z2 + a0;
        let b1 = a3 * z2 + a2;
        let b2 = bc10 * z2 + a4;
        let c0_inner = b1 * z4 + b0;
        let yb = b2 * z8 + c0_inner;

        let mask = z.simd_lt(split_z);
        f32x16::blend(mask, yn, yb)
    };
    ...
    for chunk in &mut iter {
        ...
        // 1/2.4 gamma encode → Y' via the 2-piece sqrt-substituted polynomial.
        let r_p = eval_pow_inv24(r);
        let g_p = eval_pow_inv24(g);
        let b_p = eval_pow_inv24(b);
        ...
        // BT.1886 EOTF (^2.4): degree-7 monomial via Estrin's method.
        let r2 = r_prime_out * r_prime_out;
        let r4 = r2 * r2;
        let r_h7 = c7 * r_prime_out + c6;
        let r_h5 = c5 * r_prime_out + c4;
    ...
    for px in iter.into_remainder().iter_mut() {
        // Scalar tail uses the same 2-piece poly so the SIMD body and tail
        // match bit-for-bit at chunk boundaries (no `libm::powf` divergence
        // between body and tail).
        let r_p = eval_pow_inv24_2piece_scalar(px[0].max(0.0));
        let g_p = eval_pow_inv24_2piece_scalar(px[1].max(0.0));
        let b_p = eval_pow_inv24_2piece_scalar(px[2].max(0.0));
        let y_p = LR_2020 * r_p + LG_2020 * g_p + LB_2020 * b_p;
        if y_p <= 0.0 {
            *px = [0.0, 0.0, 0.0];
            continue;
