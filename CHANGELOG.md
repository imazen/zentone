...
- `experimental::Aces2OutputTransform` — the ACES 2.0 Output Transform ported
  ...
  chroma compression, and analytic-hull gamut compression with per-display
  tables built at construction, forward and inverse. `Aces2Config` picks the
  peak luminance and limiting primaries (`Chromaticities::{REC709, P3_D65,
  REC2020, AP0, AP1}`); ACES2065-1 in, display-linear RGB out (1.0 = 100
  cd/m²). Implements `ToneMap` (`map_rgb` = forward). Every stage is held to
  OpenColorIO 2.5.2's built-in ACES 2.0 fixed functions on 222–296-sample
  goldens (`reference-checks/golden/aces2_ocio_*.csv`, generated by
...
- `experimental::Aces2DisplayTransform` + `Aces2DisplayEncoding` +
  ...
  files): peak clamp, white scaling for D60-limited gamuts encoded on D65
  displays, limiting→encoding primaries, DCDM linear scale, and the sRGB /
  gamma 2.2 / gamma 2.6 / BT.1886 / ST 2084 / HLG (BT.2390 §7 at 1000 nits)
  inverse EOTFs, forward and inverse. Ready-made encodings (`SRGB`,
  `REC709_BT1886`, `REC2100_ST2084`, `REC2100_HLG`, `DCDM_GAMMA26`, …),
  `Chromaticities::{REC709_D60, P3_D60, REC2020_D60, XYZ_E}`, and
  `transform_id()` returning the preset's `urn:ampas:aces:transformId:…`
  for any of the 54 shipped presets (`ACES_OUTPUT_TRANSFORM_IDS`).
  Implements `ToneMap` with encoded code values out. Held to OpenColorIO
  2.5.2's `ACES-OUTPUT - … _2.0` + `DISPLAY - …` builtins on 9 presets ×
  25 samples, forward to 3e-3 code value and inverse to 2e-2 of pixel
  magnitude (`reference-checks/golden/aces2_ocio_display_encoding{,_inv}.csv`).
- `experimental::aces` — `acescct_encode/decode`, `acescc_encode/decode`
  (S-2016-001 / S-2014-003, with `ap0_to_ap1` / `ap1_to_ap0`), `AscCdl`
  (slope / offset / power / saturation, ASC-clamped and no-clamp styles,
  forward and inverse), and `ReferenceGamutCompression` (ACES 1.3 RGC with
  the spec parameters as `ACES_1_3`, forward / inverse, row helpers). All
  held to OpenColorIO 2.5.2 to 2e-5 (`reference-checks/golden/aces_ocio_*.csv`,
  generated by `reference-checks/aces_ocio_encodings_reference.py`). Also
  `bradford_matrix` / `chromatic_adaptation_matrix` (Bradford or CAT02 cone
  spaces) and `rgb_to_rgb_matrix` (primaries-to-primaries with Bradford
  white adaptation) from `Lib.Academy.ColorSpaces.ctl`, checked against the
  ACES 1.x `D60_2_D65_CAT` and `AP0_2_REC709_MAT` constants.
